  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (oauthError) {
        ...
        const name = urlParams.get('name') || 'Instagram Account';
        const handle = urlParams.get('handle') || '@instagram';
        ...
        if (!igFound) {
          updated.unshift({
            ...
            name,
            handle,
            avatarUrl: avatar || 'https://images.unsplash.com/photo-1611262588024-d12430b98920?w=150&auto=format&fit=crop&q=80',
            connected: true,
            connectedAt: new Date().toISOString(),
            followerCount: followers || 23100,
        ...
        setNotification({ type: 'success', message: `Successfully connected Instagram Business account "${name}"!` });
        window.history.replaceState({}, '', window.location.pathname);
        return;
