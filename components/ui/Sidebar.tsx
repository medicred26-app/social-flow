...
// Navigation items for Customer/Business users
const CUSTOMER_NAV_ITEMS: NavItem[] = [
  ...
  { name: 'My Projects', href: '/projects', icon: ClipboardList },
  { name: 'Analytics', href: '/analytics', icon: BarChart3 },
  { name: 'Connected Accounts', href: '/accounts', icon: Share2 },
  { name: 'Settings', href: '/settings', icon: Settings },
];

// Navigation items for Freelancer users
const FREELANCER_NAV_ITEMS: NavItem[] = [
  { name: 'Dashboard', href: '/freelancer', icon: LayoutGrid },
  { name: 'My Jobs', href: '/freelancer/jobs', icon: Briefcase },
  { name: 'Active Projects', href: '/freelancer/projects', icon: ClipboardList },
  { name: 'Earnings', href: '/freelancer/earnings', icon: DollarSign },
  { name: 'My Profile', href: '/freelancer/profile', icon: UserIcon, badge: 'Pro' },
...
export function Sidebar() {
  ...
  const NAV_ITEMS = isFreelancer ? FREELANCER_NAV_ITEMS : CUSTOMER_NAV_ITEMS;

  const handleToggleRole = () => {
    if (!user) return;
    const newRole: 'customer' | 'freelancer' = isFreelancer ? 'customer' : 'freelancer';
    const updated = { ...user, role: newRole };
    localStorage.setItem('sf_auth_user', JSON.stringify(updated));
    window.location.href = newRole === 'freelancer' ? '/freelancer' : '/dashboard';
  };

  return (
    <aside className="w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-200 flex flex-col justify-between h-screen sticky top-0 z-40 transition-colors duration-200" suppressHydrationWarning>
      <div className="flex-1 overflow-y-auto sidebar-scrollbar min-h-0 flex flex-col justify-between" suppressHydrationWarning>
        <div>
          {/* Brand Header */}
        <Link href="/" title="Go to public home page" className="h-16 flex items-center px-6 border-b border-slate-200 dark:border-slate-800/80 gap-3 hover:opacity-90 transition-opacity">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500 dark:text-slate-400 font-medium tracking-wide uppercase mt-0.5">
              {isFreelancer ? 'Freelancer Portal' : 'Automation Suite'}
            </p>
          </div>
        </Link>
        ...
        <div className="px-4 pt-3 pb-1">
          <button
            ...
            title={isFreelancer ? "Switch to Customer Account" : "Switch to Freelancer Account"}
            className={`w-full flex items-center justify-between px-3 py-2 rounded-xl text-[11px] font-bold tracking-wide border transition-all hover:scale-[1.02] cursor-pointer shadow-sm ${
              isFreelancer
          ...
          >
            <div className="flex items-center gap-2">
              {isFreelancer ? (
                <><UserCheck className="w-3.5 h-3.5" /> Freelancer Mode</>
              ) : (
                <><Sparkles className="w-3.5 h-3.5" /> Customer Mode</>
              )}
            </div>
            <span className="text-[9px] px-1.5 py-0.5 rounded-md bg-slate-200 dark:bg-slate-800 text-slate-600 dark:text-slate-300 font-medium">
              Switch ⇄
            </span>
          </button>
        </div>
        ...
        <nav className="p-4 space-y-1.5">
          <div className="px-3 text-[11px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2">
            {isFreelancer ? 'Freelancer Menu' : 'Main Menu'}
          </div>
          {NAV_ITEMS.map((item) => {
